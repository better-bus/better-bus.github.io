export const fakeSeatAssignments = (rows: number, ridersPerBench: number): Record<string, string | null | undefined> => ({
    // by default a seat exists but is not assigned (null)
    ...['A', 'B', 'C', 'D', 'E', 'F']
      .flatMap((l) => new Array(rows)
        .fill(null)
        .map((_, i) => `${i + 1}${l}`))
...
// `
  ...
  .map(line => line.trim())
  .reduce((acc, curr, i) => {
    const col = i % (ridersPerBench * 2);
    const row = Math.floor(i / (ridersPerBench * 2));
